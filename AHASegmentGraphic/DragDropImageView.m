...
NSString *kPrivateDragUTI = @"com.DanielSchroth.imageDragDrop";
...
- (NSArray *)namesOfPromisedFilesDroppedAtDestination:(NSURL *)dropDestination
{
    ...
    NSInteger counter = 1;
    NSString *filename = self.path.lastPathComponent ?: @"image.tiff";
    while ([[NSFileManager defaultManager] fileExistsAtPath:[[dropDestination path] stringByAppendingPathComponent:filename]])
    {
        filename = self.path.lastPathComponent ?: [NSString stringWithFormat:@"image%ld.tiff", counter];
        counter++;
    }
